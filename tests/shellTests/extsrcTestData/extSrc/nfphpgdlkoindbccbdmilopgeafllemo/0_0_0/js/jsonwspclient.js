...
function JSONWSPClient() {
	...
	this.postRequest = function (data, response_callback, error_callback) {
	    var req = new XMLHttpRequest();
	    req.open("POST", this.url, true);
	    req.setRequestHeader("Content-Type", "application/json; charset=utf-8");
	    //eq.setRequestHeader("Content-Length", data.length);
		...
		CountlyLog.logEvent("Post: " + this.url)
		req.send(data); 
	};

	this.sendRequest = function (url, response_callback, error_callback) {
	    var req = new XMLHttpRequest();
		...
		CountlyLog.logEvent("GET: " + url);
		req.send(null);
	};
	
	this.sendAsyncRequest = function (url, response_callback) {
	    var req = new XMLHttpRequest();
	...
	this.callMethod = function (method_name, args, mirror, cb, er_cb) {
	...
	this.loadDescription = function (url, onsuccess, onError) {
	    var obj = this;
	    this.sendRequest(url, function (jsonrpc_desc) {
	        ...
	        for (method in jsonrpc_desc.methods) {
	            eval("obj." + method + "=function(args,mirror,callback,error_callback) {obj.callMethod('" + method + "',args,mirror,callback,error_callback)}");
	        }
	        if (onsuccess != null) {
	            onsuccess();
	        }
